function [out,out_split] = BarrOpt_fitfn_separate(x,BarrIndex,v)
...
%         prctile(v{2,strcmp('Infrastructure Failure Risk',v(1,:))}.OT_RISK(1:datarange),95);
    ...
    out_split(1)=-sum(v{2,1}.OT_RISK((BarrIndex(1:datarange,1)'>=2e4) + (x(1:datarange)==1)>1));
    out_split(2)=-sum(v{2,1}.OT_RISK((BarrIndex(1:datarange,1)'<2e4) + (x(1:datarange)==1)>1));
...
if ismember('Dam Utilities',v(1,:))
    ...
    out_split(9)=sum(v{2,3}.Cpcty_kW(x(1:datarange)==1));
    out_split(10)=sum(v{2,3}.RsvrVolEst(x(1:datarange)==1));
...
if ismember('Decision Costs',v(1,:))
    ...
    out_split(11)=-sum(v{2,strcmp('Decision Costs',v(1,:))}.DnCost(x(1:datarange)==0));
end
%
